...
const POISONED_MUTEX_MESSAGE: &str = "failed to unlock mutex due to another thread panicking while holding it";
...
pub fn download(id_list: Vec<String>, dir: PathBuf, max_concurrent_downloads: NonZeroUsize) -> DownloadObserver {
...
impl DownloadObserver {
    ...
    pub fn set_max_concurrent_downloads(&self, n: NonZeroUsize) {
        self.info.max_concurrent_downloads.store(n.get(), Ordering::Release);
    }
}
...
impl DownloadUpdater {
    ...
    pub fn add_ongoing_download(&self, id: String) {
        self.info.ongoing_downloads.lock()
            .expect(POISONED_MUTEX_MESSAGE)
            .push(id);
    }

    pub fn remove_ongoing_download(&self, id: String) {
        let mut ongoing_downloads = self.info.ongoing_downloads.lock()
            .expect(POISONED_MUTEX_MESSAGE);
        if let Some(index) = ongoing_downloads.iter().position(|ongoing_id| ongoing_id == &id) {
            ongoing_downloads.swap_remove(index);
        }
    }

    pub async fn add_failure(&self, id: String, err: APIErr) {
        self.info.failed_downloads.lock().expect(POISONED_MUTEX_MESSAGE).push((id, err));
    }
