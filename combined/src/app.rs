...
impl BeatSharerApp {
    fn download(&mut self) {
        ...
        // Download
        let mut contents = reqwest::blocking::get(format!(
            "https://beat-sharer-default-rtdb.firebaseio.com/{}.json?auth={}",
            self.download_code,
            dotenv!("secret")
        ))
        .unwrap()
                    ...
                    ui.horizontal(|ui| {
                        ...
                        // ID input and download button
                            ...
                            if self.download_error {
                                ui.add(
                                    egui::TextEdit::singleline(&mut self.download_code_buf)
                                        .hint_text("Bad ID")
                                        .desired_width(75.0),
                                );
                            } else {
                    ...
                    ui.horizontal(|ui| {
                        ...
                        ui.add(
                            egui::Slider::new(
                                ...
                                1..=std::thread::available_parallelism().unwrap().get() / 2,
                            )
                            .text(""),
                        );
                    });
...
fn download_songs(
    ...
    for song in download_list {
        ...
        if let Ok(contents) =
            reqwest::blocking::get(format!("https://api.beatsaver.com/maps/id/{}", song))
                .unwrap()
                .text()
        {
            ...
            if let Some(dl_start) = contents.find("downloadURL") {
                ...
                let name_end =
                    contents[name_start..contents.len() - 1].find(',').unwrap() + name_start;
                ...
                let mut file = File::create(
                    download_path
                        .clone()
                        .join(format!("{}.zip", full_name.clone())),
                )
                .unwrap();
                ...
                zip.extract(download_path.clone().join(full_name.clone()))
                    .unwrap();
                drop(zip);
